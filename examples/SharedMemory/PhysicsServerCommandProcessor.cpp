...
bool PhysicsServerCommandProcessor::processLoadClothPatchCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes)
{
    ...
    btSoftBody* psb = NULL;
    {
		...
		if (softWorld)
		{
			btSoftBodyHelpers::CreatePatch(softWorld->getWorldInfo(), corner00, corner10, corner01, corner11, numX, numY, fixed, true);
			if (!psb)
			{
				printf("Load deformable failed\n");
				return false;
	...
	}
	{
		...
		if (deformWorld)
		{
			btSoftBodyHelpers::CreatePatch(deformWorld->getWorldInfo(), corner00, corner10, corner01, corner11, numX, numY, fixed, true);
			if (!psb)
			{
				printf("Load deformable failed\n");
				return false;
